        ...
        if manager.isDemoActive, let endsAt = manager.demoEndsAt {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 1) {
                    ...
                    // The 1 Hz timeline only wraps the countdown text so the
                    // glass container, icon, and button aren't re-evaluated
                    // every second.
                    DemoCountdownText(endsAt: endsAt)
                }
...
private struct DemoCountdownText: View {
    let endsAt: Date
